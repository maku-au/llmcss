export class AiDropdownElement extends HTMLElement {
  ...
  private syncTrigger(open: boolean) {
    this.querySelectorAll('.ai-dropdown-trigger, [data-ai-toggle="dropdown"]').forEach((t) =>
  ...
  connectedCallback() {
    ...
    this.addEventListener('click', (e) => {
      const trigger = (e.target as HTMLElement).closest('.ai-dropdown-trigger');
      if (trigger && this.contains(trigger)) {
        this.toggle();
        return;
      }
      if ((e.target as HTMLElement).closest('.ai-dropdown-item')) this.close();
    });
